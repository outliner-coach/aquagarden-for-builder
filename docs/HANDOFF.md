...
## ✅ 현재 상태 (2026-07-18 최신 — 아래 이력 섹션의 '미병합'·'현재 상태' 표기를 대체함)

> **아래의 날짜별 섹션들은 시점별 append 로그다.** 각 섹션에 적힌 "main 미병합",
> "브랜치: feat-…", 테스트 개수 등은 **작성 당시** 값이며 지금은 낡았다. 실제 최신 사실은 이 블록이 기준.

- **카메라 궤도 드래그(2026-07-18, v0.10.0, 사용자 요청 "다른 각도에서도 보고 싶다")** — 수조 드래그=궤도
  회전(yaw ±25°·pitch −5~30° 클램프 — 무대 세트 성립 범위, 각도 스윕 `eval-camera-angles.png`), 더블클릭=
  정면 복귀(지수 수렴), 각도 영속(줌 패턴). FishDialogue/FoodLure를 pointerdown 즉발→`handleTap`(탭 발동)으로
  이전해 드래그와 공존(상세 CLAUDE.md "카메라 궤도" 섹션). QA 훅 `__AQUA_SET_CAMERA__`/`AQUA_SMOKE_CAM`/
  `AQUA_DEVTOOLS=1`. 라이브 QA: 사용자가 드래그 회전 직접 확인("잘 보이네요"). test **656**·lint·build·smoke pass.
- **물고기 지형 회피 + 지형 융기 반영(2026-07-18, 사용자 요청 "물고기가 지형을 뚫지 않고 피해 다니게")** —
  phase 9의 "지형을 물고기 바닥 아래로 캡핑(≤0.6)" 방식을 역전: 물고기가 위치별 로컬 바닥(지형 표면+여유고)을
  전방 예측 상승 조향+경사 수평 우회(소프트)와 y 하드 클램프(관통 원천 차단)로 회피한다(`TERRAIN_AVOID`,
  `terrainHelpers` 회피 헬퍼 4종, `Fish.update` 배선, 새우는 표면을 타고 기어오름). 이로써 산호초 마운드를
  1.5/1.2(정점 y≈−0.37, 옛 캡 −1.2)로, 다시마 기복을 0.38로 올려 phase 9 미결 ①(마운드 융기감·hardscape 45)을
  해소. 런타임 감시 `health.terrainClips`(FishSchool 관통 카운터)를 스모크가 0으로 게이트. 게이트: test
  **641**(+20: 헬퍼·Fish 3000틱 불변식·스모크 게이트)·lint·build·테마 3종 smoke(관통 0/최대 341프레임×40마리)
  통과. 캡처 비교 `eval-terrain-final-compare.png`(구 vs 신), 확대 `eval-terrain-zoom.png`.
  - **라이브 QA 확인 항목**: ① 물고기가 리프를 타넘는/돌아가는 모션 자연스러움(정적 캡처 판정 불가 —
    비전 judge는 여전히 정지컷에서 융기 체감이 약하다고 함, 52~62 노이즈 대역) ② 새우가 마운드 비탈을
    기어오르는 모습 ③ 마운드 높이·폭 취향(더 올릴 여유 있음 — 수면 여유 가드 상한 2.58).
- **테마 품질 개선 phase 9(2026-07-18, 브랜치 `feat-9-theme-quality` — v0.9.0로 main 병합·배포됨)** — 사용자
  라이브 평가 반영: 테마별 지형(terrainHelpers, 미니멀 무변위)·다시마 군락화(~86가닥·원근 레이어·황금올리브)·
  산호 리프 피복(콜로니 30+, 뭉게 주역, fan 제거). 게이트: test **621**·lint·build·smoke·비전 실채점(CLI 인증 복구)
  통과. 레퍼런스 `docs/media/reference/`. 라이브 QA 추가 항목: ① 마운드 융기감(비전 hardscape 45 — 높이 상한 0.6
  제약) ② 다시마 최외곽(양끝 ~20%) 성김 체감 ③ 산호 뭉게가 "꽃"으로 읽히는지.
- **main = `v0.9.0`** — 배경 테마 시스템(phase 8) + 테마 품질 개선(phase 9) 병합, 태그 `v0.9.0` 배포(GitHub Release 자동 빌드).
- **배경 테마 시스템 추가(2026-07-18, 브랜치 `feat-8-background-themes` — v0.9.0로 main 병합됨)** — 배경(아쿠아스케이프)을
  교체형 테마 3종(미니멀·다시마 숲·산호초)으로 확장. 상세는 `CLAUDE.md`의 "배경 테마 시스템 추가" 섹션,
  설계는 `docs/superpowers/specs/2026-07-17-background-themes-design.md` 참고. 게이트: test **572**·lint·
  build·smoke pass(테마별 `AQUA_SMOKE_THEME` 강제 전환+`health.theme` 리드백 일치, 기본 스모크는 `minimal`
  확인 — fresh 사용자 하위호환). 대표 캡처 `docs/media/themes/`. eval_vision.py는 CLI 인증 만료로 스킵
  통과(정상 — 미학은 캡처 직접 비교로 확정: 가지 산호 두께/퍼짐폭, 부채 산호 갈래폭/부채각/yaw지터 조정으로
  "마른 나뭇가지"·"성게" 인상 완화, 패널 '다시마 숲' 라벨 2줄 줌바꿈은 `white-space:nowrap`+폰트 축소로 해결).
  - **라이브(실기기) QA 미검증 항목**(사용자 동의 후 확인 권장 — 컴퓨터유즈는 사용자 컴퓨터를 점유하므로
    headless 우선 원칙): ① 테마 전환 체감(리빌드 순간 프레임 드랍 여부) ② 다시마 흔들림 자연스러움(정적
    캡처로 판정 불가 — 유닛테스트만 가드) ③ 산호 심야 무드 색감 체감 ④ 테마별 투명 슬라이더 체감
    ⑤ 패널 테마 클릭 → 앱 재시작 복원 왕복(step5가 유닛테스트·코드 추적으로만 검증, 실제 GUI 왕복 미확인).

- (이전 릴리스) **v0.8.0** — 사용자 관점 라이브 QA 평가에서 나온 개선 13건 반영(`fix-7-eval-feedback` 병합).
  상세는 `CLAUDE.md`의 "사용자 평가 피드백 반영 (2026-07-17)" 섹션 참고. 게이트: **test 486**·lint·build·
  smoke pass(frames 82 — 30fps 캡 동작 확인), 무드 시간대별 캡처(13/19/23.5h) 픽셀 비교로 틴트·감광 확인.
- **스모크가 결정적이 됨**: smoke 모드는 시작 시 localStorage를 비운다. 상태는 훅(`AQUA_SMOKE_FISH` 등)으로 구동.
- **라이브(실기기) QA 미검증 항목** — 다음 세션에서 사용자 동의 후 확인 권장(컴퓨터유즈는 사용자 컴퓨터를
  점유하므로 headless 우선, 라이브는 예약/동의 후):
  ① 30fps 캡의 유영 부드러움 체감 ② 실기기 무드 전환 체감(특히 심야) ③ 새우 0.6 크기 적절성
  ④ 투과 ON 상태 패널 조작(펼침 중 일시 해제 UX) ⑤ ⌥⌘A 단축키 충돌 여부 ⑥ CPU 절감 폭(프로덕션 빌드).
