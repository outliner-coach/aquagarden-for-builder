...
- **창 조작** — 플로팅 버튼 드래그로 이동(화면 밖으로는 안 나감), 모서리 그립으로 크기 조절, 패널 자동 위/아래 열기
- **메뉴바 트레이** — 🐠 아이콘으로 보이기/숨기기·위치 초기화·로그인 시 시작·종료 (버튼을 못 찾아도 복구 가능)
- **설정 자동 저장** — 개체수·밝기·투명도·창 위치/크기가 재시작 후에도 유지됨
...
3. 패널에서 개체수·밝기·투명도 등을 조절하고, **먹이주기/놀래키기**를 켠 뒤 화면을 클릭해 물고기와 상호작용합니다.
4. **물고기를 클릭**하면 대사가 나옵니다.
5. 플로팅 버튼을 **드래그**하면 창을 옮기고, 수조의 **오른쪽·아래·우하단 모서리를 드래그**하면 크기가 바뀝니다.
6. 다른 작업에 방해되면 **마우스 투과**를 켜 수조 영역 클릭이 뒤 화면으로 통과되게 하거나, **수조 숨김**으로 잠시 끌 수 있습니다.
7. 종료는 패널 하단 **종료** 버튼을 두 번 누릅니다.
8. 사용법이 헷갈리면 패널 우상단의 **`?` 버튼**으로 안내를 볼 수 있습니다.
