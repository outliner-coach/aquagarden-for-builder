...
- 숨김 상태에선 렌더 루프를 멈춰 절전, 표시 중에도 30fps 캡·저전력 GPU 힌트로 저부하, 물고기는 풀링으로 재사용
...
- **시간대 반응(무드)** — 켜면 시각에 따라 조명 무드가 1.5초에 걸쳐 부드럽게 변합니다. 심야는 어둑한 문라이트 블루, 저녁은 골든 앰버 — 수초·커스틱까지 함께 물듭니다
- **수중 분위기** — 절차적 커스틱(모래·물고기·돌에 투사), 깊이 틴트, 기포·글로우 스프라이트
- **하드스케이프** — 수초 5종·돌·유목이 바닥을 채우고, 깊이에 따라 먼 바닥이 수중 헤이즈로 자연스럽게 흐려짐
- **배경 테마** — 수조 배경을 미니멀·다시마 숲·산호초 3종 중에서 선택합니다. 다시마 숲은 울퉁불퉁한 암반 바닥 위에 리본형 다시마가 무리지어 서 있고, 산호초는 모래에서 **솟아오른 리프 마운드**를 가지·뇌·뭉게 산호가 뒤덮습니다. 선택은 재시작 후에도 유지됩니다
- **지형 회피 유영** — 물고기가 리프 마운드·암반 지형을 인지하고 **타넘거나 옆으로 돌아서** 헤엄칩니다(지형 관통 없음 — 런타임 검증 포함). 청소부 새우는 지형 비탈을 타고 기어오릅니다
- **인터랙션** — 물고기를 클릭하면 어종별 대사(종마다 10개, 말풍선 꼬리가 화자를 가리킴), 먹이주기/놀래키기 (마우스 투과·수조 숨김 중에는 비활성 표시, 20초 미사용 시 자동 해제)
- **확대(줌)** — 마우스 휠 또는 슬라이더로 수조를 1~2배 확대해 물고기를 가까이서 감상
- **카메라 각도** — 수조를 **드래그**하면 카메라가 좌우·위 궤도로 돌아 다른 각도에서 감상할 수 있습니다(더블클릭으로 정면 복귀, 각도는 재시작 후에도 유지)
- **토큰 사용량 표시** — 내 Claude Code 사용량·한도를 패널의 **링 게이지 2종**(5시간·주간, %와 리셋 시각, 색이 여유→주의→임박으로 변화)으로 보여주고, **물고기를 클릭**하면 어종 개성을 담아 알려줍니다(남은 기간 대비 과속이면 '아껴 써'로 톤이 바뀌고, 5시간·주간을 번갈아 언급). 실행 중인 Claude 세션에서 사용량을 읽어오며, 못 읽는 동안에는 마지막으로 받아온 값을 **흐리게 + "N분/N시간 전 기준"** 으로 보여줍니다(오래된 값을 현재값처럼 보여주지 않습니다). **토큰은 로컬에서만 쓰이고 저장·화면 노출·전송하지 않습니다.**
- **창 조작** — 플로팅 버튼 드래그로 이동(여러 모니터에 걸쳐 이동 가능, 작업 영역 밖으로는 안 나감), 모서리 그립으로 크기 조절, 패널 자동 위/아래 열기
- **메뉴바 트레이 + 복구 단축키** — 트레이 아이콘으로 보이기/숨기기·위치 초기화·로그인 시 시작·종료. 트레이가 메뉴바 혼잡(노치 맥북)으로 안 보여도 **⌥⌘A**(윈도우 `Ctrl+Alt+A`)로 창을 상단으로 복구
...
6. 수조를 **드래그**하면 카메라가 돌아가 리프·물고기를 다른 각도에서 볼 수 있습니다. **더블클릭**하면 정면으로 부드럽게 복귀합니다.
7. 플로팅 버튼을 **드래그**하면 창을 옮기고, 수조의 **오른쪽·아래·우하단 모서리를 드래그**하면 크기가 바뀝니다.
8. 다른 작업에 방해되면 **마우스 투과**를 켜 수조 영역 클릭이 뒤 화면으로 통과되게 하거나, **수조 숨김**으로 잠시 끌 수 있습니다. (이때는 먹이주기·놀래키기·확대·카메라 회전이 비활성화됩니다.)
9. 종료는 패널 하단 **종료** 버튼을 두 번 누릅니다.
10. 사용법이 헷갈리면 패널 우상단의 **`?` 버튼**으로 안내를 볼 수 있습니다.
11. 창이나 버튼이 안 보이면 **⌥⌘A**(윈도우 `Ctrl+Alt+A`)를 누르세요 — 화면 상단으로 돌아옵니다.
...
| 카메라 각도 | 수조 드래그로 좌우·위 궤도 회전, 더블클릭으로 정면 복귀 (패널 항목이 아닌 수조 제스처) |
| 시간대 반응 | 시각에 따라 조명 무드 변화(심야 청색·저녁 앰버). 기본 꺼짐 |
| 배경 테마 | 수조 배경을 미니멀·다시마 숲·산호초 중에서 선택 |
| 토큰 사용량 | 내 Claude Code 5시간·주간 사용률을 링 게이지로 표시(켜기/끄기). 물고기 클릭 시에도 알려줌. 못 읽으면 '연결 안 됨' 또는 마지막 값을 흐리게 + 기준 시각과 함께 |
| 수조 숨김 | 렌더링을 멈춰 절전(플로팅 버튼만 남음) |
| 마우스 투과 | 수조 영역 클릭을 뒤쪽 화면으로 통과. 패널이 열려 있는 동안은 일시 해제 |
| Always on Top | 항상 다른 창 위에 표시 |
| 먹이주기 / 놀래키기 | 켠 뒤 화면을 클릭하면 물고기가 반응 |
| 크기 조절 | 수조의 오른쪽·아래·우하단 모서리를 드래그 |
| 종료 | 두 번 눌러 종료 |

> 먹이주기·놀래키기·확대·카메라 회전은 **마우스 투과**나 **수조 숨김**이 켜져 있으면 동작할 수 없어 패널에서 비활성(흐리게)으로 표시됩니다.
