...
- **배경 테마** — 수조 배경을 미니멀·다시마 숲·산호초 3종 중에서 선택합니다. 다시마 숲은 리본형 다시마가 가장자리·뒤쪽에 무리지어 서 있고, 산호초는 가지·뇌·부채 산호가 밝은 산호모래 위에 어우러집니다. 선택은 재시작 후에도 유지됩니다
...
10. 창이나 버튼이 안 보이면 **⌥⌘A**(윈도우 `Ctrl+Alt+A`)를 누르세요 — 화면 상단으로 돌아옵니다.
