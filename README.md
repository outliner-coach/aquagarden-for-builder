...
- **토큰 사용량 표시** — 내 Claude Code 사용량·한도를 패널의 **링 게이지 2종**(5시간·주간, %와 리셋 시각, 색이 여유→주의→임박으로 변화)으로 보여주고, **물고기를 클릭**하면 어종 개성을 담아 알려줍니다(남은 기간 대비 과속이면 '아껴 써'로 톤이 바뀌고, 5시간·주간을 번갈아 언급). 실행 중인 Claude 세션에서 사용량을 읽어오며, 잠깐 못 읽어도 마지막 값을 '방금/N분 전'으로 유지합니다. **토큰은 로컬에서만 쓰이고 저장·화면 노출·전송하지 않습니다.**
...
| 토큰 사용량 | 내 Claude Code 5시간·주간 사용률을 링 게이지로 표시(켜기/끄기). 물고기 클릭 시에도 알려줌. 못 읽으면 '연결 안 됨'/마지막 값 |
