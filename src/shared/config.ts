...
export const TOKEN = {
  ...
  criticalPct: 0.95,
  /** 페이스 판정에 필요한 최소 사용률(0..1) — 그 이하는 페이스와 무관하게 여유로 본다. */
  pacePctFloor: 0.1,
  /** 창 경과 비율이 이 값 미만이면 페이스 판정을 보류한다(창 초반 투영 노이즈 방지). */
  paceMinElapsed: 0.03,
  /** 리셋 시점 예상 사용률(=pct/경과비율)이 이 값 이상이면 과속 → 주의로 올린다. */
  paceWarnProjection: 1.2,
