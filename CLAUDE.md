...
### 진행 예정 phase (2026-05-24 설계)
- `phases/3-quick-fixes`: ① 밝기↑ 시 중앙 네모 선(IBL `RoomEnvironment` 반사) 버그 수정, ② 유리 테두리 라인 옅게(`GLASS_EDGE_OPACITY`), ③ 비-물고기 투명 슬라이더(`sceneOpacity`), ④ 창 크기 슬라이더(`SET_WINDOW_SIZE` IPC + 캔버스 리프레임).
- `phases/4-fish-interactions`(투과 OFF에서만 동작): ① 어종/수초 레지스트리로 확장성 정리, ② 물고기 클릭 랜덤 대사(어종당 ≥10), ③ 먹이주기/놀래키기(armed 모드 + 클릭 지점 끌림/도망 + 먹이 입자).
